function robocon_arena_sim_nav_final
% =======================================================================
% ROBOCON 2026 – 3D SIMULATION + TIME-AWARE HYBRID A* NAVIGATION
%   - 3D arena + KFS blocks with oracle-bone textures
%   - Real data folders:
%        dataset\real  → 4 R2 real scroll KFS
%        dataset\R1    → 3 R1 scroll KFS (edges/corners only)
%        dataset\fake  → 1 fake KFS
%   - Remaining blocks empty
%   - All KFS share one color (red or blue) per randomization
%   - Simulated HSV+ML vision and occlusion
%   - Grid-based, time-aware navigation planner (collect + exit)
%   - Autonomous/manual toggle with 'N' key
% =======================================================================
% Controls:
%   W/S : Move Forward/Backward           (manual)
%   A/D : Strafe Left/Right               (manual)
%   Q/E : Rotate robot Left/Right         (manual)
%   SPACE: Climb to current block         (manual)
%   C    : Descend to ground              (manual)
%   Z/X  : Manual camera pitch -/+        (manual)
%   I/K  : Camera pan (yaw)               (manual)
%   V    : Toggle auto-tilt               (camera centers KFS)
%   N    : Toggle autonomous navigation   (planner)
%   R    : Reset robot
%   T    : Randomize KFS
...
% Convert mm heights to 20/40/60 grid heights
heightMap = zeros(ROWS, COLS);
for k = 1:numel(forest_blocks)
    b = forest_blocks(k);
    heightMap(b.row, b.col) = b.h / 10; % 200/400/600 -> 20/40/60
end
...
kfs_images      = {};
kfs_colors      = {};
global_kfs_color = COLOR_KFS_BASE_BLUE;
...
% ================= NAVIGATION STATE (grid-based) =================

CAPACITY = 2;
TEAM = "RED";
exitCells = [4 1; 4 3];
startCell = [curr_b.row, curr_b.col];

% Dynamic weights based on carry count (from improved time-aware code)
...
R2_EXIT_PENALTY  = 5.0 * R1_WAIT_EXPECTED;
R1_SOFT_PENALTY  = 0.2 * R1_WAIT_EXPECTED;

VISION_CONFIDENCE_THRESHOLD = 0.7;

pos      = startCell;  % grid cell
carry    = 0;
modeNav  = "COLLECT";
pathHist = pos;

belief           = strings(ROWS, COLS); belief(:) = "UNSEEN";
beliefConfidence = zeros(ROWS, COLS);
visitedCells     = false(ROWS, COLS);
visitedCells(pos(1),pos(2)) = true;
...
r1WaitAttempts = containers.Map;

randomizeKFS();

%% ======================= FIGURE SETUP ==============================
fig = figure('Name','ROBOCON 2026 - 3D + NAV Simulation', ...
...
while running && ishandle(fig)
    ...
    title(sprintf('Mission: %.1fs | Dist: %.1fm | Yaw: %.0f° | Cam Off: %.0f° | Auto: %d', ...
          stats.mission_time, stats.total_distance/1000, robot_yaw, camera_yaw_offset, autonomous_mode), ...
          'Color','white');
    ...
    % HUD
    text(0.02,0.98,sprintf('R2: %d/%d | R1: %d/%d | FAKE: %d/%d | InView: %d', ...
    ...
    text(0.02,0.90,sprintf('Pitch: %.0f° | Yaw: %.0f° | Auto: %d', ...
         camera_pitch,camera_yaw,autonomous_mode), ...
    ...
    title('CAMERA FEED (Vision+Nav Active)','Color','white','FontSize',14);
...
function randomizeKFS()
    ...
    trueForest(:) = "EMPTY";
    for idx = 1:numel(all_kfs_ids)
        bid = all_kfs_ids(idx);
        b   = forest_blocks(bid);
        trueForest(b.row,b.col) = kfs_types(idx);
    end
    ...
    if rand < 0.5
        global_kfs_color = COLOR_KFS_BASE_RED;
    else
        global_kfs_color = COLOR_KFS_BASE_BLUE;
    end
    kfs_colors = cell(1,numel(all_kfs_ids));
    for k = 1:numel(all_kfs_ids)
        kfs_colors{k} = global_kfs_color;
    end
...
%% ========= NAVIGATION HELPERS (TIME-AWARE HYBRID A*) =========

function [newBelief,newConf] = updatePerceptionFrom3DVision( ...
    cam_pos,cam_yaw,cam_pitch,blocks,truthForest,oldBelief,oldConf, ...
    KFS_SIZE,DET_RANGE,DET_FOV)

    newBelief = oldBelief;
    newConf   = oldConf;

    yaw_rad = deg2rad(cam_yaw);
    cam_forward = [cos(yaw_rad- pi/2), sin(yaw_rad- pi/2), 0];

    for k = 1:numel(blocks)
        b = blocks(k);
        row = b.row; col = b.col;

        center3D = [b.x,b.y,b.h + KFS_SIZE/2];
        dXY = center3D(1:2) - cam_pos(1:2);
        dist = norm(dXY);
        if dist > DET_RANGE
            continue;
        end
        ...
        angle = acosd(dot(cam_forward(1:2),dir2d));
        if angle > DET_FOV/2
            continue;
        end

        if isOccluded(cam_pos,center3D,blocks,BLOCK_SIZE,b.id)
            continue;
        end

        t = truthForest(row,col);
        if t == "EMPTY"
            newBelief(row,col) = "EMPTY";
            newConf(row,col)   = max(newConf(row,col),0.9);
        else
            newBelief(row,col) = t;
            base_conf = 0.6 + 0.3*(1 - dist/DET_RANGE);
            base_conf = max(0.6,min(0.95,base_conf));
            newConf(row,col)   = max(newConf(row,col),base_conf);
        end
    end
end

function valid = isValidCell(p)
    valid = p(1)>=1 && p(1)<=ROWS && p(2)>=1 && p(2)<=COLS;
end

function target = chooseBestR2(p,b,conf,carryCount,hMap,exits,timeLeft)
    ...
    if timeLeft < 60
        beta = beta*2.0;   % become more exit‑centric late game
    end
    ...
    for r = 1:ROWS
        for c = 1:COLS
            if b(r, c) ~= "R2" || conf(r, c) < MIN_PLAN_CONF
                continue;
            end
            ...
            % Confidence weighting: prefer higher‑confidence R2
...
function target = findBestExploreCell(p,b,conf,hMap,visited)
    % Explore UNKNOWN/UNSEEN cells, biased forward and lower terrain
    bestCost = inf;
    target = [];
    ...
    for r = 1:ROWS
        for c = 1:COLS
            ...
            cost = dist + forwardBonus + heightPenalty + confPenalty;
...
function [target,blockedR1] = planExitRoute(p,b,conf,exits,hMap)
    target = [];
    blockedR1 = [];
    ...
    for i = 1:size(exits,1)
        exitGoal = exits(i,:);
        if (b(exitGoal(1),exitGoal(2))=="R1" || b(exitGoal(1),exitGoal(2))=="FAKE") ...
           && conf(exitGoal(1),exitGoal(2)) > 0.7
            continue;
        end
        path = astarWithTerrain(p,exitGoal,b,conf,hMap,"EXIT");
        if ~isempty(path)
    ...
    for i = 1:size(exits,1)
        e = exits(i,:);
        if b(e(1),e(2))=="R1" && conf(e(1),e(2))>=VISION_CONFIDENCE_THRESHOLD
            blockedR1 = e;
            return;
        end
    end
...
function cost = estimateTerrainCost(from,to,hMap)
    % Manhattan + height change cost using T_UP/T_DOWN
...
function path = astarWithTerrain(start,goal,b,conf,hMap,currentMode)
    ...
    while ~isempty(open) && iterations < maxIterations
        ...
        for i = 1:size(neighbors,1)
            ...
            if currentMode=="EXIT"
                if cellType=="R1"
                    if cellConf>VISION_CONFIDENCE_THRESHOLD
            ...
            tentativeG = gScore(current(1),current(2)) + ...
                         moveCost(current,neighbor,hMap) + extraCost + pickCost;
...
function path = reconstructPath(cameFrom,current)
    ...
    pathLength = 1;
    while isKey(cameFrom,key(current)) && pathLength < maxPathLength
...
function w = interpWeight(carryCount,w0,wFull)
    if carryCount == 0
        w = w0;
...
function nav_step()
    % One autonomous navigation step (called each frame)
    elapsedTime   = toc(gameStartTime);
    remainingTime = TIME_LIMIT - elapsedTime;

    if remainingTime < TIME_BUFFER && ~emergencyExitTriggered
    ...
    persistent lastGridPos;
    if isempty(lastGridPos), lastGridPos = pos; end
    if norm(pos - lastGridPos) < POSITION_TOLERANCE
        stuckCounter = stuckCounter + 1;
        if stuckCounter >= MAX_STUCK_COUNT
            fprintf("[NAV] Stuck, re-evaluating...\n");
            if modeNav=="COLLECT" && carry>0
                modeNav = "EXIT";
            elseif modeNav=="EXIT"
                fprintf("[NAV] Already EXIT, will keep trying.\n");
            end
            stuckCounter = 0;
            replanCounter = replanCounter + 1;
            if replanCounter >= MAX_REPLAN_ATTEMPTS
    ...
    % Update grid belief from 3D vision
    [belief,beliefConfidence] = updatePerceptionFrom3DVision( ...
        camera_pos,camera_yaw,camera_pitch, ...
        forest_blocks,trueForest,belief,beliefConfidence, ...
        KFS_SIZE,DETECTION_RANGE,DETECTION_FOV);

    visitedCells(pos(1),pos(2)) = true;

    % Immediate pick if confident R2 under robot
    if belief(pos(1),pos(2))=="R2" && ...
       beliefConfidence(pos(1),pos(2))>=VISION_CONFIDENCE_THRESHOLD

        carry = carry + 1;
        trueForest(pos(1),pos(2)) = "EMPTY";
        belief(pos(1),pos(2))     = "EMPTY";
        collectedR2Positions = [collectedR2Positions; pos]; %#ok<AGROW>
    ...
    % Exit reached?
    if ismember(pos,exitCells,'rows')
        if modeNav=="EXIT" || carry>=CAPACITY
            fprintf("[NAV] Reached exit at (%d,%d) with %d items\n",pos,carry);
            autonomous_mode = false;
            return;
        end
    end

    % R1 waiting logic (from improved code)
    if modeNav=="WAIT_FOR_EXIT"
        keysR1 = r1WaitStart.keys;
        if isempty(keysR1)
            modeNav = "EXIT";
        else
            kR1 = keysR1{1};
            elapsedR1 = toc(r1WaitStart(kR1));
            if elapsedR1 >= WAIT_TIME
                if rand <= R1_CLEAR_PROB
                    rc = sscanf(kR1, '%d,%d');
                    trueForest(rc(1), rc(2)) = "EMPTY";
                    belief(rc(1), rc(2)) = "EMPTY";
                    remove(r1WaitStart, kR1);
    ...
    if modeNav=="COLLECT"
        % Best R2 using hybrid cost
        target = chooseBestR2(pos,belief,beliefConfidence,carry,heightMap,exitCells,remainingTime);

        if isempty(target)
            % Explore unknown/unseen regions
            target = findBestExploreCell(pos,belief,beliefConfidence,heightMap,visitedCells);
            if isempty(target)
                if carry>0
                    fprintf("[NAV] NO R2 & NO UNKNOWN → EXIT MODE\n");
                    modeNav = "EXIT";
                else
                    fprintf("[NAV] Nothing to do, stopping auto\n");
                    autonomous_mode = false;
                end
                return;
            else
    ...
    elseif modeNav=="EXIT"
        [target,blockedR1] = planExitRoute(pos,belief,beliefConfidence,exitCells,heightMap);
        if isempty(target) && ~isempty(blockedR1)
            ...
            if ~isKey(r1WaitStart, r1key)
                ...
                fprintf("[NAV] EXIT BLOCKED by R1 at (%d,%d) → WAITING\n", blockedR1);
            end
            modeNav = "WAIT_FOR_EXIT";
            return;
    ...
    % Execute one grid move
    if ~isempty(target)
        path = astarWithTerrain(pos,target,belief,beliefConfidence,heightMap,modeNav);
        if isempty(path) || size(path,1)<2
            fprintf("[NAV] No path to (%d,%d)\n",target);
            return;
        end
        nextCell = path(2,:);
        if abs(nextCell(1)-pos(1)) + abs(nextCell(2)-pos(2)) ~= 1
            fprintf("[NAV] Non-adjacent step, skipping\n");
            return;
        end

        % Move robot in 3D to that block's center
        idxBlock = sub2ind([ROWS,COLS],nextCell(1),nextCell(2));
        bNext = forest_blocks(idxBlock);
        robot_pos(1:2) = [bNext.x,bNext.y];
        if robot_pos(3) > 10
            robot_pos(3) = bNext.h;
        end
        current_block_id = bNext.id;
        pos = nextCell;
        pathHist(end+1,:) = pos; %#ok<AGROW>
        fprintf("[NAV] Step to cell (%d,%d)\n",pos);

        if recording_path
            path_history(end+1,:) = robot_pos; %#ok<AGROW>
        end
    end
end
