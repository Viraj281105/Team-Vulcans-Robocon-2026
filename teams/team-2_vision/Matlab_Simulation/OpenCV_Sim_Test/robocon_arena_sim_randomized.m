function robocon_arena_sim_randomized
% =======================================================================
% ROBOCON 2026 – KUNG FU QUEST
% R2 MEIHUA FOREST SIMULATOR (RANDOMIZED KFS + TILTED CAMERA + WIDE FOV)
% =======================================================================
...
% =======================================================================
    ...
    CAMERA_PITCH = 15;      % 15 Degrees Downward Tilt
    CAMERA_RANGE = 3000;
    ...
    % --- KFS CONFIGURATION (DYNAMIC) ---
    % These variables are now updated by the randomizer function
    all_kfs_ids = [];
    kfs_types = [];
    total_r2_kfs = 4;
    ...
    function randomizeKFS()
        % Randomly assign R1, R2, and Fake to the 12 blocks
        pool = randperm(12); % Random shuffle of 1 to 12
        
        % Rules: 3 R1, 4 R2, 1 Fake
        r1_idx = pool(1:3);
        r2_idx = pool(4:7);
        fake_idx = pool(8);
    ...
    function keyHandler(~, event)
        ...
        switch event.Key
            ...
            case 't'
                randomizeKFS(); % Trigger randomization
